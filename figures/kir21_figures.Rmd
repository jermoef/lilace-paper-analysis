...
kir21_method_df <- distinct(kir21_rescaled_fit_df_called[c("hgvs", "total_counts",
...
colnames(kir21_method_df) <- c("hgvs", "total_counts", "enrich", "enrich_pval", "enrich_pval_disc", "enrich_syn_sd_disc", "weight_mean", "weight_mean_disc", "weight_mean_BH_disc", "weight_mean_se")
...
tsv_kir21$weight_mean_BH <- tsv_kir21$weight_mean
model_weight_scatter <- plot_method_scatter(tsv_kir21, "effect", "weight_mean_BH", "Lilace", "weighted bin", F)
print(model_weight_scatter)
...
p_gof <- ggplot(tsv_kir21, aes(x=position)) + 
  ...
  theme(axis.title.x = element_blank(), axis.text.x = element_blank()) +
  theme(plot.margin = unit(c(0,0,0,0), "cm"), text = element_text(size = 25), axis.text = element_text(size = 20))
...
tsv_kir21$effect_both <- ifelse(tsv_kir21$discovery05 != 0 & tsv_kir21[[comp_method]], 1, 0)
tsv_kir21$effect_comp <- ifelse(tsv_kir21$discovery05 == 0 & tsv_kir21[[comp_method]], 1, 0)
tsv_kir21$effect_lilace <- ifelse(tsv_kir21$discovery05 != 0 & !tsv_kir21[[comp_method]], 1, 0)
tsv_kir21$effect_neither <- ifelse(tsv_kir21$discovery05 == 0 & !tsv_kir21[[comp_method]], 1, 0)
tsv_kir21$cov_call_cat <- as.factor(ifelse(tsv_kir21$effect_lilace, "Lilace", 
                                    ifelse(tsv_kir21$effect_comp, 
                                           comp_name, ifelse(tsv_kir21$effect_both, "both", "neither"))))
call_colors <- setNames(c('#4e79a7', '#76b7b2', '#f28e2b', '#e15759'),# '#e15759'
                          c("both", "neither", "Lilace", comp_name))
tsv_kir21$cov_call_cat <- relevel(tsv_kir21$cov_call_cat, "Lilace")
tsv_kir21$cov_call_cat <- relevel(tsv_kir21$cov_call_cat, comp_name)
tsv_kir21$cov_call_cat <- relevel(tsv_kir21$cov_call_cat, "both")
tsv_kir21$cov_call_cat <- relevel(tsv_kir21$cov_call_cat, "neither")
tsv_kir21_cov <- tsv_kir21

# tsv_kir21_cov[tsv_kir21_cov$cov_call_cat == "Lilace" | tsv_kir21_cov$cov_call_cat == comp_name,]
p <- ggplot(tsv_kir21_cov, aes(x=total_counts, fill=cov_call_cat)) + geom_histogram(alpha=0.65, position = "stack", color="black", bins=25) + 
...
print(p)
ggsave(paste0("data_figures/kir21/", pheno, "_", comp_method, "_coverage.png"), p, height=6, width=9)

call_colors <- setNames(c('#4e79a7', '#76b7b2', '#f28e2b', '#e15759'),# '#e15759'
                          c("both", "neither", "Lilace", comp_name))
p <- ggplot(tsv_kir21_cov, aes(x=weight_mean_se, fill=as.factor(cov_call_cat))) + geom_histogram(alpha=0.65, position = "stack", color="black", bins=25) + 
...
ggsave(paste0("data_figures/kir21/", pheno, "_", comp_method, "_se.png"), p, height=6, width=9)
...
for (pheno in c("Abundance", "surface_abundance")) {
  ...
  kir21_method_df <- distinct(kir21_rescaled_fit_df_called[c("hgvs", 
                                                           "enrich_score", "enrich_pval_adjusted", "enrich_score_disc", "enrich_score_syn_sd_disc",
                                                           "weight_effect_mean", "weight_effect_mean_syn_sd_disc", "weight_effect_mean_syn_sd_BH_disc")])
  colnames(kir21_method_df) <- c("hgvs", "enrich", "enrich_pval", "enrich_pval_disc", "enrich_syn_sd_disc", "weight_mean", "weight_mean_disc", "weight_mean_BH_disc")
  tsv_kir21 <- merge(tsv_kir21, kir21_method_df)
  ...
  # comparison plots
  if (pheno == "surface_abundance") pheno <- "Surface"
  tsv_kir21$enrich_disc <- tsv_kir21$enrich_pval_disc
  model_enrich_scatter <- plot_method_scatter(tsv_kir21, "effect", "enrich", "Lilace", "Enrich2", F, paste0("Enrich2 (p-val) vs Lilace ", pheno))
  print(model_enrich_scatter)
  ggsave(paste0("data_figures/kir21/", pheno, "_scatter_enrich_pval.png"), model_enrich_scatter, width=6.75, height=4.5)
  ...
  print(model_enrich_scatter)
  ggsave(paste0("data_figures/kir21/", pheno, "_scatter_enrich_2sd.png"), model_enrich_scatter, width=6.75, height=4.5)
  ...
  print(model_weight_scatter)
  ggsave(paste0("data_figures/kir21/", pheno, "_weight_2sd.png"), model_weight_scatter, width=6.75, height=4.5)
  ...
  tsv_kir21$weight_mean_BH <- tsv_kir21$weight_mean
  model_weight_scatter <- plot_method_scatter(tsv_kir21, "effect", "weight_mean_BH", "Lilace", "Mean bin (BH)", F, paste0("Mean bin (BH) vs Lilace ", pheno))
  print(model_weight_scatter)
  ggsave(paste0("data_figures/kir21/", pheno, "_weight_BH.png"), model_weight_scatter, width=6.75, height=4.5)
...
tsv_kir21_all <- tsv_kir21_all %>% pivot_wider(id_cols=c("hgvs", "wildtype", "position", "mutation", "type"), names_from="exp", 
                                               values_from=c("effect", "effect_se", "lfsr", "pos_effect", "pos_effect_se", "pos_sd", "pos_sd_se", 
                                                             ...
                                                             "enrich_syn_sd_disc", "weight_mean", "weight_mean_disc", "weight_mean_BH", "weight_mean_BH_disc"))
...
is_effect_cols <- tsv_kir21_all[,c("weight_mean_BH_disc_abundance", "weight_mean_BH_disc_surface_abundance")]
names(is_effect_cols) <- c("abundance", "surface")
tsv_kir21_all$weight_mean_BH_pheno_class <- apply(is_effect_cols, 1, function(x) {
  if (any(is.na(x))) {
    return(NA)
  } else {
    return(paste(names(is_effect_cols)[x], collapse = ", "))
...
tsv_kir21_all$effect_vs_weight_mean_BH_pheno_class <- tsv_kir21_all$effect_pheno_class == tsv_kir21_all$weight_mean_BH_pheno_class
...
syn_surf_cutoff_top <- min(tsv_kir21_all[tsv_kir21_all$weight_mean_BH_disc_surface_abundance & tsv_kir21_all$weight_mean_BH_surface_abundance > 0,]$weight_mean_BH_surface_abundance, na.rm=T)
syn_surf_cutoff_bot <- max(tsv_kir21_all[tsv_kir21_all$weight_mean_BH_disc_surface_abundance & tsv_kir21_all$weight_mean_BH_surface_abundance < 0,]$weight_mean_BH_surface_abundance, na.rm=T)
syn_abun_cutoff_top <- min(tsv_kir21_all[tsv_kir21_all$weight_mean_BH_disc_abundance & tsv_kir21_all$weight_mean_BH_abundance > 0,]$weight_mean_BH_abundance, na.rm=T)
syn_abun_cutoff_bot <- max(tsv_kir21_all[tsv_kir21_all$weight_mean_BH_disc_abundance & tsv_kir21_all$weight_mean_BH_abundance < 0,]$weight_mean_BH_abundance, na.rm=T)

tsv_kir21_all_plot <- tsv_kir21_all %>% arrange(desc(effect_vs_weight_mean_BH_pheno_class))
ggplot(tsv_kir21_all_plot[!is.na(tsv_kir21_all_plot$effect_vs_weight_mean_BH_pheno_class),], 
       aes(weight_mean_abundance, weight_mean_surface_abundance, col=effect_vs_weight_mean_BH_pheno_class)) + geom_point(alpha=0.5, size=2) + 
    geom_hline(yintercept=syn_surf_cutoff_bot, linetype='dashed', col = 'navy', lwd=0.5) +
    geom_hline(yintercept=syn_surf_cutoff_top, linetype='dashed', col = 'navy', lwd=0.5) +
    geom_vline(xintercept=syn_abun_cutoff_bot, linetype='dashed', col = 'navy', lwd=0.5) +
    geom_vline(xintercept=syn_abun_cutoff_top, linetype='dashed', col = 'navy', lwd=0.5) +
    theme_cowplot() + theme(legend.position="bottom", text = element_text(size = 20), axis.text = element_text(size = 20)) + scale_color_manual(values=c("TRUE" = "gray", "FALSE"="red")) +
    xlab("Weighted Mean Bin Abundance") + ylab("Weighted Mean Bin Surface") +
    guides(color=guide_legend(title="Same Classification as Lilace")) + scale_y_continuous(breaks = c(-0.5, -0.2, 0, 0.2))
ggsave("data_figures/kir21/weight_BH_pheno_comp.png", width=8, height=6)
...
bothp = 3308 / (901 + 3308 + 1050)
abundance_only = 901 / (901 + 3308 + 1050)
surface_only = 1050 / (901 + 3308 + 1050)
print(paste(bothp, abundance_only, surface_only))

tsv_kir21_all$enrich_pval_pheno_class %>% table
bothp = 4339 / (1991 + 4339 + 2164)
abundance_only = 1991 / (1991 + 4339 + 2164)
surface_only = 2164 / (1991 + 4339 + 2164)
print(paste(bothp, abundance_only, surface_only))
...
tsv_kir21_all$weight_mean_BH_pheno_class %>% table
bothp = 3448 / (1468 + 3448 + 948)
abundance_only = 1468 / (1468 + 3448 + 948)
surface_only = 948 / (1468 + 3448 + 948)
print(paste(bothp, abundance_only, surface_only))
